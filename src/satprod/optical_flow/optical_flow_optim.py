...
class OpticalFlowOptim():
    ...
    def evaluate(self, data: pd.DataFrame):
        ...
        for park in parks:
            ...
            for trig in ['sin', 'cos']:
                ...
                pix = data[f'{park}_deg_pixel_{trig}'].values
                med = data[f'{park}_deg_median_{trig}'].values
                wdi = data[f'wind_direction_{park}_{trig}'].values
            ...
            pix = data[f'{park}_mag_pixel'].values
            med = data[f'{park}_mag_median'].values
            wsp = data[f'wind_speed_{park}'].values
    ...
    def get_numerical_data(self, date: datetime) -> pd.DataFrame:
        df_nan = self.num.read_formatted_data(nan=True)
        
        start = date + timedelta(hours=4)
        stop = date + timedelta(hours=21)
        data = df_nan.loc[start.strftime('%Y-%m-%d %H:%M:%S'):stop.strftime('%Y-%m-%d %H:%M:%S')].copy()
        parks = ['bess', 'skom', 'vals', 'yvik']
        for park in parks:
            data[f'wind_direction_{park}_sin'] = np.sin(np.pi*wind_degrees_to_polar(
                data[f'wind_direction_{park}_sin'], data[f'wind_direction_{park}_cos'])/180)
            data[f'wind_direction_{park}_cos'] = np.cos(np.pi*wind_degrees_to_polar(
                data[f'wind_direction_{park}_sin'], data[f'wind_direction_{park}_cos'])/180)
        return data
    
    def get_of_data(self, date: datetime, imgType: ImgType) -> pd.DataFrame:
        date_for_name = date + timedelta(hours=3, minutes=15)
        name = date_for_name.strftime('%Y-%m-%d-%H') + f'-15min-20sc'
        of_res = pd.read_csv(f'{self.root}/data/of_num_results/{imgType.value}/{name}.csv')
